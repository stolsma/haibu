...
Drone.prototype.cleanAll = function (callback) {
  ...
  async.forEach(Object.keys(this.apps), cleanApp, function (err) {
    ...
    return err ? callback(err) : haibu.utils.rmApps(appsDir, callback);
  });
};
...
Drone.prototype._add = function (app, drone, callback) {
  ...
  var pid = drone.process.pid;
  record.drones[pid] = drone;

  //
  // In the event that the drone unexpectedly restarts,
  // we need to update the record with the new pid so that
  // we can control it later on.
  //
  drone.monitor.on('restart', function (_, data) {
    self._update(record, pid, data);
    pid = data.pid;
  });

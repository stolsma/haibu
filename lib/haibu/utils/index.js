...
utils.rmApp = function (appsDir, app, callback) {
  return rimraf(path.join(appsDir, app.name), callback);
};
...
// #### @appsDir {string} Root for all application source files.
// #### @callback {function} Continuation passed to respond to.
// Removes all source code associated with all applications
...
utils.rmApps = function (appsDir, callback) {
  ...
  fs.readdir(appsDir, function (err, apps) {
    ...
    async.forEach(apps, function rmApp (app, next) {
      rimraf(path.join(appsDir, app), next);
    }, callback);
  });
};

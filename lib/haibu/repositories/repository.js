...
Repository.prototype.validate = function (keys, app) {
  ...
  keys = keys || [];
  required = ['name', 'repository.type', 'scripts.start'].concat(keys);
...
// #### @callback {function} Continuation to pass control back to when complete.
// Creates directories for the applications associated with this repository instance.
//
...
//
// ### @appDir {string}
// Location on disk of the app that this repository instance manages.
//
Repository.prototype.__defineGetter__('appDir', function () {
  return path.join(this.appsDir, this.app.name);
});
